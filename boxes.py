...
def remove_small_boxes(boxes, min_size):
    ...
    # or:(numpy array, float) -> numpy array
    """
    ...
    Returns:
        keep (Tensor[K, 4]): boxes that have both sides
    ...
    if type(boxes) == torch.Tensor:
        ...
        ws, hs = boxes[:, 2] - boxes[:, 0], boxes[:, 3] - boxes[:, 1]  # 预测boxes的宽和高
        # keep = (ws >= min_size) & (hs >= min_size)  # 当满足宽，高都大于给定阈值时为True
        keep = np.logical_and(ws.__ge__(min_size), hs.__ge__(min_size))
        # nonzero(): Returns a tensor containing the indices of all non-zero elements of input
        # keep = keep.nonzero().squeeze(1)
...
#     Arguments:
#         boxes1 (Tensor[N, 4] or numpy)
#         boxes2 (Tensor[M, 4] or numpy)
...
def get_targetItems_by_boxes(img, bboxes):
    """
    Return list of 2d arrays which are target items .

    Boxes are expected to be in (x1, y1, x2, y2) format.

    Arguments:
        img (Tensor[M, N, 3])
        boxes2 (Tensor[P, 4] or 2d array,which shape is [P, 4])
    ...
    for index in range(bboxes.shape[0]):
        bbox = bboxes[index,:]
        targetItem = img[bbox[1]:bbox[3],bbox[0]:bbox[2],:]
        targetItems.append(targetItem)

    return targetItems

def pointInBox(x,y,bbox):
